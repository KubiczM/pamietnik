...
export function CoverPhoto({ photo, position, onPickPhoto, onSavePosition, onRemovePhoto, decoration }: Props) {
  ...
  return (
    ...
    >
      ...
      {/* ── Tryb kadrowania — osobna warstwa z touch-action: none ── */}
      {mode === 'reposition' && (
        <div
        ...
        >
          ...
          <div className="absolute bottom-4 inset-x-0 flex justify-center gap-3">
            <button
              className="pointer-events-auto px-5 py-2.5 rounded-full bg-black/40 backdrop-blur-sm text-white text-sm font-sans font-medium"
            ...
            <button
              className="pointer-events-auto px-6 py-2.5 rounded-full bg-white text-gray-900 text-sm font-sans font-semibold shadow-lg"
              onPointerDown={e => e.stopPropagation()}
              onTouchStart={e => e.stopPropagation()}
              onClick={confirm}
            >
